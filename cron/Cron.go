...
func (c *Cron) Start() error {
	if c.scheduler == nil {
		return fmt.Errorf("cron scheduler do not init")
	}
	c.scheduler.Start()
	return nil
}
...
// RegisterJobs đăng ký các handler struct có chứa các hàm cron cần gọi.
//
// Cách sử dụng:
//
// RegisterJobs đăng ký các handler struct có chứa các hàm cron cần gọi.
...
// Cách sử dụng:
//
//	cron.RegisterJobs(MyJobHandler{}, AnotherHandler{})
...
// RegisterJobWithTags đăng ký các handler struct có chứa các hàm cron cần gọi.
//
// Cách sử dụng:
//
// RegisterJobWithTags đăng ký các handler struct có chứa các hàm cron cần gọi.
...
// Cách sử dụng:
//
//	cron.RegisterJobWithTags(MyJobHandler{}, AnotherHandler{})
func (c *Cron) RegisterJobWithTags(jobs ...interface{}) {
	if len(jobs) == 0 {
		return
	}
